<?php
    ...
    if(isset($_POST["startDate"])){
        ...
        $completeInfo = array();
        $leftDates = array();
        echo $income = "SELECT * FROM `tbl_income` WHERE `post_at` >= '".date("Y-m-d", strtotime($startDate))."' && `post_at` <= '".date("Y-m-d", strtotime($endDate))."'";
        echo $expence = "SELECT * FROM `tbl_expenses` WHERE `payment_date` >= '".date("Y-m-d", strtotime($startDate))."' && `payment_date` <= '".date("Y-m-d", strtotime($endDate))."'";
        echo $incomeGetPrevious = "SELECT SUM(`amount`) AS sumIncome FROM `tbl_income` WHERE `post_at` > '".date("Y-m-d", strtotime($startDate))."'";
        echo $expenceGetPrevious = "SELECT SUM(`amount`) AS sumExpence FROM `tbl_expenses` WHERE `payment_date` > '".date("Y-m-d", strtotime($startDate))."'";
